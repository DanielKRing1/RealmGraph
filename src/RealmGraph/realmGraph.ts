...
import {
  pageRank as genericPageRank,
  getInitialWeights,
  redistributeNodeWeight,
  inflateEdgeAttrs,
} from "@asianpersonn/pagerank";
...
import {
  ...
  RealmGraph,
  RGCreateParams,
  RGDeletePropertiesParams,
  RGLoadableParams,
  RGLoadGraphParams,
  RGUpdatePropertiesParams,
} from "./types";
 ...
 * @returns
 */
export const createRealmGraph = async ({
  metaRealmPath,
  loadableRealmPath,
  graphName,
  propertyNames,
}: RGCreateParams): Promise<RealmGraph> => {
  ...
  await MetaRealm.LoadableRealmManager.reloadRealm({
    metaRealmPath,
    loadableRealmPath,
  });

  // 3. Setup RealmGraph
  const realmGraph: RealmGraph = await initializeRealmGraph({
    metaRealmPath,
    loadableRealmPath,
    graphName,
    propertyNames,
...
export const loadRealmGraph = async ({
  metaRealmPath,
  loadableRealmPath,
  graphName,
  shouldReloadRealm = true,
}: RGLoadGraphParams): Promise<RealmGraph> => {
  ...
  const graphProperties: Realm.PropertiesTypes = MetaRealm.getProperties(
    metaRealmPath,
    nodeSchemaName
  );

  // 2. Remove unrelated keys
  const KEYS_TO_RM: Set<string> = new Set([
    GRAPH_ENTITY_ID_KEY,
    EDGE_IDS_KEY,
    genCollectiveTallyName(),
  ]);
  const rawPropertyNames: string[] = Object.keys(graphProperties).filter(
    (key: string) => !KEYS_TO_RM.has(key)
  );
...
const initializeRealmGraph = async ({
  metaRealmPath,
  loadableRealmPath,
  graphName,
  propertyNames,
  ...
  let _catalystGraph: CatalystGraph = _createCatalystGraph(
    graphName,
    propertyNames,
    loadRealmSync
  );
  ...
  function getMetaRealmPath(): string {
    return metaRealmPath;
  }
  function getLoadableRealmPath(): string {
    return loadableRealmPath;
  }
  function getPropertyNames(): string[] {
    return _catalystGraph.propertyNames;
  }
  ...
  const getNode = (nodeId: string): (Realm.Object & CGNode) | undefined => {
    const loadedGraphRealm: Realm = loadRealmSync();
    return loadedGraphRealm.objectForPrimaryKey(
      genNodeSchemaName(graphName),
      nodeId
    );
  };
  ...
  const getEdge = (edgeId: string): (Realm.Object & CGEdge) | undefined => {
    const loadedGraphRealm: Realm = loadRealmSync();
    return loadedGraphRealm.objectForPrimaryKey(
      genEdgeSchemaName(graphName),
      edgeId
    );
  };

  const getGraphEntity = (
    ids: string[],
  ...
  const rate = (
    ...
    // this.realm?.write(() => {
    rateResult = _catalystGraph.rate(
      propertyName,
      nodeIds,
      rating,
      weights,
  ...
  const undoRate = (
    ...
    // Wrap in write transaction to apply updates directly to Realm objects during rate
    // this.realm?.write(() => {
    rateResult = _catalystGraph.undoRate(
      propertyName,
      nodeIds,
      rating,
      weights,
  ...
  function _getPageRankNodeMethods() {
    // 1. All nodes copy
    const allNodesRawCopy: CGNode[] = getAllNodes().map(
      (node: Realm.Object & CGNode) => node.toJSON()
    );
    // 2. Get node id
    ...
    const getNodeAttrs = (node: CGNode) =>
      DictUtils.copyDictKeep<number>(node, keysToKeep);
  ...
  function _getPageRankEdgeMethods(
    allNodesRaw: CGNode[],
    allEdgesRaw: CGEdge[] = getAllEdges().map(
      (realmEdge: Realm.Object & CGEdge) => realmEdge.toJSON()
    )
  ) {
    // 1. Node map
    const nodeMap: Dict<CGNode> = allNodesRaw.reduce(
      (nodeMap: Dict<CGNode>, node: CGNode) => {
    ...
    const edgeMap: Dict<CGEdge> = allEdgesRawCopy.reduce(
      (edgeMap: Dict<CGEdge>, edge: CGEdge) => {
    ...
    const getEdges = (node: CGNode): CGEdge[] =>
      node.edgeIds.map((edgeId: string) => edgeMap[edgeId]);

    // 4. Get edge attrs
    const keysToKeep: string[] = _getPageRankWeightKeys();
    const getEdgeAttrs = (edge: CGEdge) =>
      DictUtils.copyDictKeep(edge, keysToKeep);
    // const getEdgeAttrs = (edge: CGEdge) => {
    ...
    const getDestinationNode = (node: CGNode, edge: CGEdge): CGNode => {
      const destinationNodeId: string =
        edge.nodeId1 == node.id ? edge.nodeId2 : edge.nodeId1;
  ...
  function _getPageRankWeightKeys() {
    ...
    const keysToKeep: string[] = propertyNames.reduce(
      (keysToKeep: string[], propertyName: string) => {
  ...
  function pageRank(
    ...
    // 2. Get edge methods
    const { getEdges, getEdgeAttrs, getDestinationNode } =
      _getPageRankEdgeMethods(allNodes);

    // 3. Get initial weighted node map: Id -> pointing to each Node's weights divided by sum of all nodes' weights
    const initialMap: Dict<Dict<number>> = getInitialWeights(
      allNodes,
      getNodeId,
      getNodeAttrs
    );
  ...
  function recommend(
    ...
    const recommendationMap: Dict<Dict<number>> = recommendRank(
      centralNodeIds,
      nodeTargetCentralWeight,
      edgeInflationMagnitude,
      iterations,
      dampingFactor
    );

    const sortedRecommendations: RankedNode[] = Object.entries(
      recommendationMap
    )
      .map(([nodeId, nodeRank]) => ({
        id: nodeId,
      ...
      .sort(
        (a: RankedNode, b: RankedNode) => a[desiredAttrKey] - b[desiredAttrKey]
      );
  ...
  function recommendRank(
    ...
    const initialMapRaw: Dict<Dict<number>> = getInitialWeights(
      allNodesRawCopy,
      getNodeId,
      getNodeAttrs
    );
    ...
    const initialMapBiased: Dict<Dict<number>> = redistributeNodeWeight(
      initialMapRaw,
      nodeTargetCentralWeight,
      centralNodeIds
    );
    ...
    // 5. Get edge methods
    const { allEdges, getEdges, getEdgeAttrs, getDestinationNode } =
      _getPageRankEdgeMethods(allNodesBiased);
    const centralNodeIdSet: Set<string> = new Set(centralNodeIds);
    const isConnectedToCentralNode = (edge: CGEdge) =>
      centralNodeIdSet.has(edge.nodeId1) || centralNodeIdSet.has(edge.nodeId2);
    const allEdgesInflated: CGEdge[] = inflateEdgeAttrs(
      allEdges,
      getEdgeAttrs,
      edgeInflationMagnitude,
      isConnectedToCentralNode
    );
    const { getEdges: getEdgesInflated } = _getPageRankEdgeMethods(
      allNodesBiased,
      allEdgesInflated
    );
  ...
  async function commonlyDoneWith(targetNodeId: string): Promise<CGEdge[]> {
    const realm: Realm = await MetaRealm.LoadableRealmManager.loadRealm({
      metaRealmPath,
      loadableRealmPath,
    });

    const node: (Realm.Object & CGNode) | undefined = realm.objectForPrimaryKey(
      genNodeSchemaName(graphName),
      targetNodeId
    );
    ...
    const edges: CGEdge[] = realm
      .objects(genEdgeSchemaName(graphName))
      .toJSON()
      .filter((edge: CGEdge) => desiredEdgeIds.has(edge.id));

    return edges.sort(
      (a: CGEdge, b: CGEdge) =>
        a[genCollectiveTallyName()] - b[genCollectiveTallyName()]
    );
  }
   ...
   * @returns
   */
  async function commonlyDoneByOutput(
    nodeId: string,
    output: string
  ): Promise<CGEdge[]> {
    const realm: Realm = await MetaRealm.LoadableRealmManager.loadRealm({
    ...
    return edges.sort(
      (a: CGEdge, b: CGEdge) =>
        a[genSingleTallyName(output)] - b[genSingleTallyName(output)]
    );
  }
   ...
   * @returns
   */
  async function highlyRatedByOutput(
    nodeId: string,
    output: string
  ): Promise<CGEdge[]> {
    const realm: Realm = await MetaRealm.LoadableRealmManager.loadRealm({
    ...
    return edges.sort(
      (a: CGEdge, b: CGEdge) =>
        a[genSingleAverageName(output)] - b[genSingleAverageName(output)]
    );
  }
   ...
   * @returns
   */
  function rankMostInfluentialToCentralSet(
    centralNodeIds: string[],
    ...
    for (let nodeId of Object.keys(initialMapRaw)) {
      ...
      if (centralNodeIdSet.has(nodeId))
        initialMapRaw[nodeId] = DictUtils.mutateDict(
          initialMapRaw[nodeId],
          (attrKey: string, val: number) => 1 / centralNodeIdSet.size
        );
      // 3.2. Every other node has 0 weight
      else
        initialMapRaw[nodeId] = DictUtils.multiplyDictScalar(
          initialMapRaw[nodeId],
          0
        );
    }
  ...
  async function loadRealm(): Promise<Realm> {
    return await MetaRealm.LoadableRealmManager.loadRealm({
      metaRealmPath,
      loadableRealmPath,
    });
  }
  async function reloadRealm(): Promise<Realm> {
    return await MetaRealm.LoadableRealmManager.reloadRealm({
      metaRealmPath,
      loadableRealmPath,
    });
  }
  function loadRealmSync(): Realm {
    return MetaRealm.LoadableRealmManager.loadRealmSync({
      metaRealmPath,
      loadableRealmPath,
    });
  }
  function reloadRealmSync(): Realm {
    return MetaRealm.LoadableRealmManager.reloadRealmSync({
      metaRealmPath,
      loadableRealmPath,
    });
  }
...
const _saveGraphSchemas = (
  ...
  const baseSchema: Realm.ObjectSchema = genBaseSchema(
    graphName,
    propertyNames
  );
  ...
  const saveParams: SaveSchemaParams[] = [nodeSchema, edgeSchema].map(
...
const _deleteGraphSchemas = async ({
  metaRealmPath,
  loadableRealmPath,
  graphName,
  reloadRealm,
}: RGDeletePropertiesParams): Promise<void> => {
...
async function _updateGraphProperties({
  metaRealmPath,
  loadableRealmPath,
  graphName,
  newPropertyNames,
  reloadRealm,
}: RGUpdatePropertiesParams): Promise<void> {
  // 1. Create updated graph schemas
  const newBaseSchema: Realm.ObjectSchema = genBaseSchema(
    graphName,
    newPropertyNames
  );
...
const _createCatalystGraph = (
  ...
  return new CatalystGraph({
    ...
    getNode: (nodeId: string) => {
      ...
      const node: (Realm.Object & CGNode) | undefined =
        loadedGraphRealm!.objectForPrimaryKey(_getNodeSchemaName(), nodeId);
      return node != undefined ? node.toJSON() : undefined;
    },
    getEdge: (edgeId: string) => {
      const loadedGraphRealm: Realm = loadRealmSync();

      const edge: (Realm.Object & CGEdge) | undefined =
        loadedGraphRealm!.objectForPrimaryKey(_getEdgeSchemaName(), edgeId);
    ...
    updateNode: (newNode: CGNode) => {
      ...
      const realmNode: (Realm.Object & CGNode) | undefined =
        loadedGraphRealm.objectForPrimaryKey(_getNodeSchemaName(), newNode.id);
      // Does not exist
      if (
        !realmNode ||
        (!!realmNode && Object.keys(realmNode!.toJSON()).length === 0)
      )
        return;
    ...
    updateEdge: (newEdge: CGEdge) => {
      ...
      const realmEdge: (Realm.Object & CGNode) | undefined =
        loadedGraphRealm.objectForPrimaryKey(_getEdgeSchemaName(), newEdge.id);
      // Does not exist
      if (
        !realmEdge ||
        (!!realmEdge && Object.keys(realmEdge!.toJSON()).length === 0)
      )
        return;
